                    ...
                    <div class="card shadow-sm border-0 rounded-4 p-4 bg-white text-dark">
                        ...
                        <form @submit.prevent="updateProfile" novalidate>
                            <div class="mb-3">
                                ...
                                <input v-model="form.name" type="text" id="name" class="form-control rounded-pill"
                                    placeholder="Masukkan nama lengkap" required />
                            </div>

                            <div class="mb-3">
                                <label for="email" class="form-label">Email</label>
                                <input v-model="form.email" type="email" id="email" class="form-control rounded-pill"
                                    placeholder="Masukkan email baru" />
                            </div>

                            <div class="mb-3">
                                <label for="phone" class="form-label">Nomor Telepon</label>
                                <input v-model="form.phoneNumber" type="tel" id="phone"
                                    class="form-control rounded-pill" placeholder="Masukkan nomor telepon" />
                            </div>

                            <div class="mb-3 position-relative">
                                <label for="password" class="form-label">Kata Sandi Baru</label>
                                <input :type="isPasswordVisible ? 'text' : 'password'" v-model="form.password"
                                    id="password" class="form-control rounded-pill"
                                    placeholder="Biarkan kosong jika tidak diubah" />
                                <span @click="togglePasswordVisibility"
                                    class="position-absolute top-50 end-0 translate-middle pe-3 cursor-pointer">
