        ...
        <div class="col-md-8 col-lg-6">
          <div class="card shadow-sm border-0 rounded-4 p-4 bg-white text-dark position-relative">
            ...
            <div class="text-center mb-4">
              ...
              <h3 class="mt-2 fw-bold">{{ tukang.name }}</h3>
              <p class="text-muted">Tukang</p>
            </div>

            <!-- Profile Info -->
            <div class="profile-info mb-4">
              <p><strong>Email:</strong> {{ tukang.email }}</p>
              <p><strong>Nomor Telepon:</strong> {{ tukang.phoneNumber || 'N/A' }}</p>
              <p><strong>Status Ketersediaan:</strong>
                <span :class="[
                  'badge ms-2',
                  tukang.availability ? 'bg-success' : 'bg-danger'
                ]">
                  {{ tukang.availability ? 'Tersedia' : 'Sedang Dipesan' }}
                </span>
              </p>
               <p><strong>Total Rating:</strong> {{ tukang.totrating.toFixed(2) }} ⭐ ({{ tukang.ratingCount }} ulasan)</p>
            </div>
...
export default {
  ...
  methods: {
    ...
    async confirmDeleteAccount() {
      const confirmed = window.confirm("Apakah Anda yakin ingin menghapus akun? Aksi ini tidak dapat dibatalkan.");
      if (!confirmed) return;

      try {
        const res = await fetch('http://localhost:8080/api/tukang/deleteAccount', {
          method: 'DELETE',
          credentials: 'include'
        });
    ...
    goToEdit() {
      this.$router.push('/Profile/Tukang/Edit');
    },
